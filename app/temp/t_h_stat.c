...
static u8 t_div;            // 温度曲线分度(整数):每字符对应的 0.1°C 数(1 = 0.1°C/字符,2 = 0.2°C/字符)
...
// 故负温用减),只做整数比较、不逐点浮点。分度是整数除数 t_div(每字符 = t_div × 0.1°C):
// 极差 ≤2.0°C → t_div = 1(0.1°C 阶距恒 1 字符);>2.0°C → t_div = 2(0.2°C/字符);
// 极差过大 → 增大 t_div 使曲线 ≤ 50 字符。位置 = 偏移量 ÷ t_div 向下取整(整数除法):
// 0.0/0.1 同长、0.2/0.3 同长、…,而 .9 与下一个整度(.0)必不同长。最小值默认映射到
// 第 20 列,若 20 + 宽度 > 50,则把最大值映射到第 50 列(最小值随之收缩到 2 ~ 20 列)。
...
static void t_h_curve_calc(void)
{
    ...
    s16 v10_range = (s16)(v10_max - v10_min); // 极差,单位 0.1°C
    // 分度为整数除数档位:每字符对应 t_div 个 0.1°C。极差 ≤2.0°C 时 1 字符 = 0.1°C
    // (0.1°C 阶距恒 1 字符),>2.0°C 时 1 字符 = 0.2°C。位置一律向下取整(整数除法):
    // 0.0/0.1 同长、0.2/0.3 同长、…,而 .9 与下一个整度(.0)必不同长。
    // 极差过大时增大除数使曲线不超过 50 字符。极差 = 0 时除数仍为 1,所有点画在基准列
    t_div = 1;
    if (v10_range > 20) t_div = 2;
    if ((u16)v10_range / t_div > 50) t_div = (u8)(((u16)v10_range + 49) / 50);
    u8 scaled = (u8)((u16)v10_range / t_div); // 曲线最大宽度(字符)
    t_base = (20 + scaled <= 50) ? 20 : (u8)(50 - scaled);
    ...
    else
    {
        ...
        else
        {
            ...
            if (th_k < (u16)snap_cnt)
            {
                ...
                s8 ti = t_h_snap[i].ti;
                u8 td = t_h_snap[i].td;
                s16 rel = (s16)((s16)ti * 10 + ((ti < 0) ? -(s16)td : (s16)td) - t_min_v10) / t_div;
                u8 pos = (u8)(t_base + rel);
                if (pos < 2) pos = 2;
                if (pos > 50) pos = 50;
...
void t_h_stat_test(void)
{
    ...
    // 随机选择数据范围,覆盖曲线显示的各种分支(温度小数 td 随机,极差 = 整数跨度 + 0~9 阶距):
    //   0: 20~29°C / 40~60%  温度 >2°C 档(0.2°C/字符)45~49 字符,20+宽度>50 基准收缩;
    //                         湿度极差 20% 默认 1%/字符
    //   1: 24~24°C / 55~60%  温度 ≤2°C 档(0.1°C/字符)0~9 字符,无放大无最小钳位;
    //                         湿度极差 5% < 10 放大到 0.5%/字符
    //   2: -5~35°C / 10~90%  温度极差 >10°C,t_div 增大,曲线 ≤50 字符;
    //                         湿度极差 80% > 50 压缩到 2%/字符
    //   3: 25~29°C / 40~60%  温度 >2°C 档 20~24 字符,基准 20
    //   4: -20~5°C / 10~90%  负温标注(带符号小数)+ 极差 >10°C 上限
    //   5: 27~28°C / 40~90%  温度 ≤2°C 档 10~19 字符,0.1°C 恒 = 1 字符,相邻值永不同长
    //                         (复现 27.0/26.9 同长的报告场景);湿度极差 50% 边界 50 字符 + 基准收缩
    //   6: 25~32°C / 10~90%  温度 >2°C 档 35~39 字符,0.0/0.1 同长、0.8/0.9 同长,
    //                         .9 与下一整度必不同长
    u8 sc = (u8)(rand() % 7);
    s8 t_lo, t_hi;
    u8 h_lo, h_hi;
    switch (sc)
    {
        case 0: t_lo = 20; t_hi = 29; h_lo = 40; h_hi = 60; break;
        case 1: t_lo = 24; t_hi = 24; h_lo = 55; h_hi = 60; break;
        case 2: t_lo = -5; t_hi = 35; h_lo = 10; h_hi = 90; break;
        case 3: t_lo = 25; t_hi = 29; h_lo = 40; h_hi = 60; break;
        case 4: t_lo = -20; t_hi = 5; h_lo = 10; h_hi = 90; break;
        case 5: t_lo = 27; t_hi = 28; h_lo = 40; h_hi = 90; break;
        default: t_lo = 25; t_hi = 32; h_lo = 10; h_hi = 90; break;
    }
